...
| `SECRET`           | [Secret key](https://docs.djangoproject.com/en/stable/ref/settings/#secret-key) used for cryptographic signing. Should be a random string of at least 32 bytes (e.g. `openssl rand -hex 32`). The JWT signing key is derived from it via SHA-256. |
